...
import { Home, Calendar, FileText, MessageCircle, LogIn, User, LogOut } from "lucide-react"
    ...
    <>
          ...
          <div className="grid grid-cols-5 gap-1">
            ...
            {isAuthenticated ? (
              ...
              >
                ...
                <span className="text-xs text-muted-foreground truncate">Sortir</span>
              </button>
            ) : (
              <Link
                href="/auth/login"
              ...
              >
                ...
                <span className="text-xs truncate">Connexion</span>
              </Link>
            )}
          </div>
        </div>
      ...
      >
        ...
        >
          ...
          >
            <div
              className={cn("w-6 h-0.5 bg-muted-foreground transition-all duration-300", isCollapsed && "rotate-90")}
            />
          </button>
              ...
              if (!isAuthenticated && item.href.startsWith("/dashboard")) {
                return (
                  <button
                    ...
                    title={`${item.label} (Connexion requise)`}
                  >
                    <Icon className="h-5 w-5 flex-shrink-0" />
                    <span
                      className={cn(
                        "transition-opacity duration-300 whitespace-nowrap text-sm font-medium",
                        isCollapsed ? "opacity-0 w-0" : "opacity-0 group-hover:opacity-100",
                      )}
                    >
                      {item.label}
                    </span>
